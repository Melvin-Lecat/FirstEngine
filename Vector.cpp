...
Vector Vector::Normalize() { return Vector(x, y, z) * (1 / Vector::Norme()); } // A v�rifier quand m�me
Vector Vector::Vectoriel(Vector v) {
	return Vector(y * v.z - z * v.y, z * v.x - x * v.z, x * v.y - y * v.x); // A v�rifier quadn m�me
}